...
const setupApp = () => {
  if (onboarding.value && !initialization.value) {
    lastSavedStep.value = localStorage.getItem("save-step");
    let transcript =
    ...
    if (lastSavedStep.value) {
      transcript +=
        "Anda juga dapat melanjutkan sesi anda sebelumnya dengan menggunakan tombol di bawah.";
    }
...
watch(onboarding, setupApp);
watch(initialization, setupApp);
...
<template>
  <div
    ...
    class="relative h-[100svh] w-[100svw] bg-slate-900 bg-cover"
  >
    ...
    <div v-else>
      ...
      <HowToUseView
        v-if="howToUseModal"
        @close="howToUseModal = !howToUseModal"
      />
      <CompleteView
        v-if="currentStep === dataScenario.length - 1"
        @complete="emit('endApp')"
      />
    </div>
  </div>
