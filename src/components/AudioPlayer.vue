<script setup>
  ...
  const audioTimeUpdate = () => {
    audioProgress.value = Math.round((audioRef.value.currentTime / audioRef.value.duration) * 100);
  };
...
<template>
  <div class="flex flex-col items-center gap-3">
    <div class="relative h-2 w-full rounded-full bg-slate-300" @click="progressClick">
      <div class="h-full rounded-full bg-slate-900" :style="`width: ${audioProgress}%`"></div>
    </div>
    <button class="flex items-center justify-center gap-2 text-sm font-semibold" @click="toggleSound">
      <audio ref="audioRef">
